...
module.exports = {
    ...
    toDockerCompose: function(cargoConfig) {
        ...
        return yaml.stringify(dockerCompose, 20, cargoConfig.spaces ? cargoConfig.spaces : 2)
    },
    ...
    getService: function(config, name, service) {
        return {
            image: config.dockerImagePrefix + '/' + name,
            ports: service.ports.map(port => `${port}:${port}`),
            networks: [config.network.name],
            environment: service.env,
            restart: 'always',
            depends_on: [config.database.toLowerCase() + '-0'].concat(service.dependsOn ? service.dependsOn : [])
        }
    },
